...
template <typename T,
	...
	static const U stackCapacity =
			utility::round_to_multiple<SABOT_UINTN>(sizeof(SABOT_UINTN), stackCapacityT);
 ...
 public:
	...
	void add(const T* const buf, const U count) {
		...
		if(newSize < capacity()) {
			...
			const U newCapacity =
					utility::round_to_multiple<SABOT_UINTN, sizeof(SABOT_UINTN)>(newSize);
